...
void Moteur::render(int size) 
{
    ...
    for (int i = 0; i < size; i++) {
        if (gameObjectList[i]->toDisplay()) {
            Vertice* vertToDisaply =  gameObjectList[i]->findComponent<Vertice>();
            if (vertToDisaply != nullptr)  continue;
            ...
            //d3ddev->DrawIndexedPrimitive(vertToDisplay->getPrimitivMethode(), 0, 0, vertToDisplay->getnumvert(), 0, vertToDisplay->getprimcount());
